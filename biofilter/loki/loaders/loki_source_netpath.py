		...
		with zipfile.ZipFile('NetPath_GeneReg_TSV1.zip','r') as pathZip:
			...
			for info in pathZip.infolist():
				...
				if info.filename == 'NetPath_Gene_regulation_all.txt':
					...
					header = pathFile.__next__().rstrip()
					if not header.decode().startswith("Gene regulation id	Pathway name	Pathway ID	Gene name	Entrez gene ID"): #	Regulation	Experiment	PubMed ID
						self.log(" ERROR\n")
