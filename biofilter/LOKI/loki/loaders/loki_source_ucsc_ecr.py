	...
	def getRegions(self, f, options):
		...
		while not EOF:
			...
			if 0: # running-average metric with minSize bugs (original algorithm)
				while sn <= sx:
					s0,s1 = sn,sn
					while (s1 < sx) and ((sum(segments[s][2] for s in range(s0,s1+2)) / sum(segments[s][3] for s in range(s0,s1+2))) >= minIdent):
						s1 += 2
					if s1 == sx:
				...
				while sn <= sx:
					s0,s1 = sn,sn
					while (s1 < sx) and ((sum(segments[s][2] for s in range(s0,s1+3)) / sum(segments[s][3] for s in range(s0,s1+3))) >= minIdent):
						s1 += 2
					if (segments[s1][1] - segments[s0][0] + 1) >= minSize:
						regions.append( (segments[s0][0],segments[s1][1]) )
					sn = s1+2
				...
				while partitions:
					...
					s0,s1 = sn,sx
					while (s0 < s1) and ((sum(segments[s][2] for s in range(s0,s1+1)) / sum(segments[s][3] for s in range(s0,s1+1))) < minIdent):
						sw = [s1-1]
